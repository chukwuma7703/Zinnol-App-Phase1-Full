...
describe('AnnualResult Tests', () => {
  ...
  describe('Core Functionality', () => {
    ...
    it('should enforce unique constraints', async () => {
      ...
      await request(app).post('/api/annualresult').send(data);
      
      // Try to create duplicate
      const response = await request(app).post('/api/annualresult').send(data);
  ...
  describe('Security', () => {
    ...
    it('should enforce rate limiting', async () => {
      const requests = Array(100).fill().map(() => 
        request(app).get('/api/annualresult')
      );
